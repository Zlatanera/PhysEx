...
@objc protocol MotionManagerDelegate: class {
...
class MotionManager {
    ...
    private var latestMotionData: CMDeviceMotion?
    private var latestSensorData : [String : simd_double3] = [:]  // 3 vector
        ...
        if motionManager.isDeviceMotionAvailable {
            ...
            motionManager.startDeviceMotionUpdates(using: .xTrueNorthZVertical, to: .main) { [weak self] (deviceMotion, error) in
                ...
                if let err = error as NSError? {
                    ...
                    if deviceMotion != nil {
                        self.updateMotionData(data: deviceMotion!)
